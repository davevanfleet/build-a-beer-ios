...
struct BeerStyleDetailView: View {
    ...
    var body: some View {
        ScrollView {
            ...
            if let sampleRecipe = beerStyle.sampleRecipe {
                NavigationLink("\(Image(systemName: "pencil")) Play with sample recipe for this style") {
                    RecipeBuilderView(recipeFormValues: sampleRecipe.recipeFormValues)
                }.padding()
            }
